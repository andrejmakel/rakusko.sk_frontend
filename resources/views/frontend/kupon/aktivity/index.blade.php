  ...
  <div class="col-6">
    <div class="card" style="height: 100px">
       <div class="card-body bg-danger">
        <p class="card-text text-center fw-bolder text-white text-uppercase">{{ $activity->activity_sk ?? '' }}</p>
        <a href="{{ url($activity->slug_sk ?? '' ) }}" class="stretched-link"></a>
       </div>
    </div>
  </div>
  @endforeach
...
@foreach($activities as $key => $activity)
            <div class="col">
                <div class="card text-white text-center">
                    ...
                    <h3 class="card-title">{{ $activity->activity_sk ?? '' }}</h3>
                    <a href="{{ url($activity->slug_sk ?? '' ) }}" class="stretched-link"></a>
                    </div>
                </div>
            </div>
